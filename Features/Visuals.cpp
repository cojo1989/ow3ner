...
void CVisuals::PlayerESP(int index)
{
	...
	if (Vars.Visuals.InfoWeapon) {
		...
		D::DrawString(F::ESP, x + (w / 2), y + h + 10, Color::White(), FONT_CENTER, mews);
	}
...
void CVisuals::Watermark()
{
	...
	if (InGame() && G::LocalPlayer->GetAlive())
		D::DrawString(F::Revue, 5, 500, G::LastLBYUpdate ? Color(200, 0, 0) : Color(0, 200, 0), FONT_LEFT, XorStr("LBY"));
}
...
void CVisuals::HealthBar(int x, int y, int w, int h, float health, float armor)
{
	...
	int Width = (h * (hp / 100.f));

	int Red = 255 - (hp*2.55);
	int Green = hp*2.55;


	D::DrawRect(x - 5, y, 5, h, Color(10, 10, 10, 150));
	D::DrawRect(x - 4, y + 1, 3, Width - 2, Color(Red, Green, 0, 255));
	...
	D::DrawRect(x + w, y, 5, h, Color(10, 10, 10, 150));
	D::DrawRect(x + w + 1, y + 1, 3, tits - 2, Color(0, 0, 255, 255));
