...
void CClantagChanger::UpdateClanTagCallback()
{
	if (strlen(Vars.Misc.ClantagChanger.value) > 0 && Vars.Misc.ClantagChanger.type > STATIC)
	{
		...
		{
		case MARQUEE:
			*CClantagChanger::animation = CClantagChanger::Marquee(charenc("MARQUEE"), ct);
			break;
		case WORDS:
			*CClantagChanger::animation = CClantagChanger::Words(charenc("WORDS"), ct);
			break;
		case LETTERS:
			*CClantagChanger::animation = CClantagChanger::Letters(charenc("LETTERS"), ct);
			break;
		}
	...
	CClantagChanger::animations = {
		CClantagChanger::Marquee(charenc("WRS"), charenc("xSniper")),
		CClantagChanger::Words(charenc("WRS"), charenc("Privat Romanian Cheat")),
		CClantagChanger::Letters(charenc("WRS"), charenc("REKT"))
	};
...
void CClantagChanger::BeginFrame()
{
	...
	long currentTime_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
	static long timeStamp = currentTime_ms;
	if (currentTime_ms - timeStamp > Vars.Misc.ClantagChanger.animation_speed)
	{
