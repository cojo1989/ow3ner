...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	// Draw frame
	RenderFrame(frame_bb.Min, frame_bb.Max - ImVec2(0, 9), GetColorU32(ImGuiCol_FrameBg), true, style.FrameRounding);
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), grab_bb.Max + ImVec2(0, 0) - ImVec2(0, 9), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderTextClipped(frame_bb.Min, frame_bb.Max + ImVec2(0, 11), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

	if (label_size.x > 0.0f)
		RenderText(ImVec2((frame_bb.Max.x - frame_bb.Min.x) / 2, frame_bb.Max.y + style.FramePadding.y + 14), label);
...
bool ImGui::CheckboxText(const char* label)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
//bool ImGui::Combo(const char * label, int * current_item, bool(*items_getter)(void *data, int idx, const char **out_text), void * data, int items_count, int height_in_items, ImVec2 size)
//{
//	ImGuiWindow* window = GetCurrentWindow();
//	if (window->SkipItems)
//		return false;
...
//	RenderCollapseTriangle(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y) + style.FramePadding, IsPopupOpen(id) ? true : false);
//
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float popup_opened_height)
{
	...
	const ImRect value_bb(frame_bb.Min, frame_bb.Max - ImVec2(arrow_size, 0.0f));
	RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(hovered ? (ImGuiCol_FrameBg + ImColor(20, 20, 20, 0)) : ImGuiCol_FrameBg), true, style.FrameRounding);
	// RenderFrame(ImVec2(frame_bb.Max.x-arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), true, style.FrameRounding); // FIXME-ROUNDING
	//RenderCollapseTriangle(ImVec2(frame_bb.Max.x-arrow_size, frame_bb.Min.y) + style.FramePadding, true);
	//window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 1, frame_bb.Min.y + 4), ImVec2(frame_bb.Max.x - 1, frame_bb.Min.y + 4), ImColor(210, 210, 210, 255));
	//window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 1, frame_bb.Min.y + 8), ImVec2(frame_bb.Max.x - 1, frame_bb.Min.y + 8), ImColor(210, 210, 210, 255));
	//window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 1, frame_bb.Min.y + 12), ImVec2(frame_bb.Max.x - 1, frame_bb.Min.y + 12), ImColor(210, 210, 210, 255));
	//window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 1, frame_bb.Min.y + 40), ImVec2(frame_bb.Max.x - 1, frame_bb.Min.y + 40), ImColor(240, 240, 240, 255), 2.f);
