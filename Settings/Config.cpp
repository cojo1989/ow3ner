...
void CConfig::Setup()
{
	...
	SetupValue(Vars.Visuals.bonebox, false, strenc("Visuals"), strenc("Box.Head"));
	...
	SetupValue(Vars.Misc.Watermark, false, strenc("Misc"), strenc("Watermark"));
	...
	Vars.Misc.ClantagChanger.value = strdup("WRS Cheats");
...
void CConfig::Save()
{
	...
	{
		char szCmd[256];
		sprintf(szCmd, "\\WRS\\%s.ini", Vars.Misc.configname);

		folder = std::string(path) + strenc("\\WRS\\");
